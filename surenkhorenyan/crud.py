...
async def create_orders_and_products(session: AsyncSession):
    ...
    keyboard = await create_product(
        session=session, name="Keyboard", description="Mechanical keyboard", price=2000
    )

    display = await create_product(
        session=session, name="Display", description="4K Display", price=30000
    )
...
async def create_gift_product_for_existing_orders(session: AsyncSession):
    ...
    gift_product = await create_product(
        session=session, name="Gift Card", description="Gift Card for you", price=0
    )
    for order in orders:
        order.products_details.append(
            OrderProductAssociation(count=1, unit_price=0, product=gift_product)
        )
...
async def main_relations(session: AsyncSession):
    ...
    await create_posts(
        session, user_sam.id, "FastAPI intro", "FasAPI Advanced", "FastAPI more"
    )
